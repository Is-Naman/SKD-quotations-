...
function Invoke-WinRtAsync($asyncOp, $typeInput) {
    ...
    if ($typeInput -is [string]) {
        # Strip brackets if passed as "[TypeName]" string
    ...
    # Find the correct AsTask method for IAsyncOperation<TResult>
...
# StoreAsync returns IAsyncOperationWithProgress<uint32, uint32>
